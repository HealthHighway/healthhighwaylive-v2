...
class MCategory extends React.Component {
            ...
            <View style={{flex:1, backgroundColor:'#fff'}} >
                ...
                <ScrollView style={{flex:1}}>
                    <View style={{height:scale(60),backgroundColor:'white', flexDirection:'row',paddingHorizontal:scale(20), ...styles.shadowStyle}} >
                        <BackHeaderWithTitleCentered onBackPress={() => this.props.navigation.goBack()} showBackBtn={true} showTitle={true} title="Mindfulness"  />
                    </View>
                    ...
                    <ProblemCheckBox 
                        ...
                        myProblem={"Stress Reduction"}
                        onPress={(myProblem) => this.handleSetUnsetProblem(myProblem)}
                    />

                    <ProblemCheckBox 
                        problems={this.state.problems}
                        myProblem={"Self Exploration"}
                        onPress={(myProblem) => this.handleSetUnsetProblem(myProblem)}
                    />

                    <ProblemCheckBox 
                        problems={this.state.problems}
                        myProblem={"Motivation"}
                        onPress={(myProblem) => this.handleSetUnsetProblem(myProblem)}
                    />

                    <ProblemCheckBox 
                        problems={this.state.problems}
                        myProblem={"Physical & Emotional Health Booster"}
                        onPress={(myProblem) => this.handleSetUnsetProblem(myProblem)}
                    />

                    <ProblemCheckBox 
                        problems={this.state.problems}
                        myProblem={"Reset Biological Clock"}
                        onPress={(myProblem) => this.handleSetUnsetProblem(myProblem)}
                    />
...
export default connect(null, { addPrivateInfo })(MCategory)
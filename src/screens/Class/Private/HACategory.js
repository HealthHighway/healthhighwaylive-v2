...
class HACategory extends React.Component {
            ...
            <View style={{flex:1, backgroundColor:'#fff'}} >
                ...
                <ScrollView style={{flex:1}}>
                    <View style={{height:scale(60),backgroundColor:'white', flexDirection:'row',paddingHorizontal:scale(20), ...styles.shadowStyle}} >
                        <BackHeaderWithTitleCentered onBackPress={() => this.props.navigation.goBack()} showBackBtn={true} showTitle={true} title="Heal Ailments"  />
                    </View>
                    ...
                    <ProblemCheckBox 
                        problems={this.state.problems}
                        myProblem={"Eye Problems"}
                        onPress={(myProblem) => this.handleSetUnsetProblem(myProblem)}
                    />

                    <ProblemCheckBox 
                        problems={this.state.problems}
                        myProblem={"Bone & Joint Pains"}
                        onPress={(myProblem) => this.handleSetUnsetProblem(myProblem)}
                    />

                    <ProblemCheckBox 
                        problems={this.state.problems}
                        myProblem={"Thyroidism"}
                        onPress={(myProblem) => this.handleSetUnsetProblem(myProblem)}
                    />

                    <ProblemCheckBox 
                        problems={this.state.problems}
                        myProblem={"Breathing Issue"}
                        onPress={(myProblem) => this.handleSetUnsetProblem(myProblem)}
                    />
...
export default connect(null, { addPrivateInfo })(HACategory)
...
class LWCategory extends React.Component {
            ...
            <View style={{flex:1, backgroundColor:'#fff'}} >
                ...
                <ScrollView style={{flex:1}}>
                    <View style={{height:scale(60),backgroundColor:'white', flexDirection:'row',paddingHorizontal:scale(20), ...styles.shadowStyle}} >
                        <BackHeaderWithTitleCentered onBackPress={() => this.props.navigation.goBack()} showBackBtn={true} showTitle={true} title="Lose Weight"  />
                    </View>
                    ...
                    <ProblemCheckBox 
                        ...
                        myProblem={"Weight Management"}
                        onPress={(myProblem) => this.handleSetUnsetProblem(myProblem)}
                    />

                    <ProblemCheckBox 
                        problems={this.state.problems}
                        myProblem={"Belly Fat Reduction"}
                        onPress={(myProblem) => this.handleSetUnsetProblem(myProblem)}
                    />

                    <ProblemCheckBox 
                        problems={this.state.problems}
                        myProblem={"Dietary Weight Reduction"}
                        onPress={(myProblem) => this.handleSetUnsetProblem(myProblem)}
                    />

                    <ProblemCheckBox 
                        problems={this.state.problems}
                        myProblem={"Fat Burn Workout"}
                        onPress={(myProblem) => this.handleSetUnsetProblem(myProblem)}
                    />
...
export default connect(null, { addPrivateInfo })(LWCategory)